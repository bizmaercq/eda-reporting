...
and am.MATURITY_DATE> = '01-jan-2018'
...
and ah.bkg_date>= all (select h.bkg_date  from actb_accbal_history h where ah.account = y.cust_ac_no and h.bkg_date <='31-DEC-2018')
...
select distinct aa.branch_code,aa.account,ca.ac_desc, aa.lcy_closing_bal ,ca.tod_limit,ca.tod_start_date,p.ude_value INT_RATE
from actb_accbal_history aa ,sttm_cust_account ca, gltb_cust_acc_breakup gb,ICTM_ACC_UDEVALS P
where ca.cust_ac_no=gb.cust_ac_no
and ca.cust_ac_no = aa.account
AND ca.CUST_AC_NO=P.ACC
AND P.UDE_VALUE<>'0'
AND P.UDE_ID='OD_RATE' 
and ca.tod_start_date between '01-JUL-2018' and '31-DEC-2018' 
and aa.bkg_date =
...
select distinct  r.branch_code,r.cust_ac_no,y.ac_desc,aa.acy_closing_bal,y.tod_limit,y.tod_start_date 
from gltb_cust_acc_breakup r, sttm_cust_account y,actb_accbal_history aa
where /*r.period_code='M12' and*/ r.fin_year='FY2017' and r.dr_bal_lcy<>'0'
and y.tod_start_date between '01-Jul-2018' and '31-dec-2018'
and aa.account = y.cust_ac_no
--AND p.ude_id='OD_RATE' AND R.CUST_AC_NO=P.ACC
and r.cust_ac_no=y.cust_ac_no
and aa.bkg_date =
(select max(a.bkg_date) from actb_accbal_history a 
...
and y.fin_year='FY2019' and y.cr_bal_lcy<>'0'
and s.ude_id='INT_RATE'
and x.TD_Maturity_date > '30/06/2019' --between '01/01/2017' and '30/06/2017'
...
SELECT ACCOUNT_NO,
 ...
 WHERE /*TRUNC(CHECKER_DT)*/TD.TD_Maturity_date BETWEEN '01/01/2019' and '30/06/2019'
 --and RECORD_STAT = 'O' 
 --DEPOSIT_AMOUNT <> MATURITY_AMOUNT
 ORDER BY DEPOSIT_AMOUNT DESC, BRANCH_NAME, PRODUCT_CODE;
...
select distinct zz.Account_no,zz.branch_code,zz.customer_Name,zz.Date_of_Open,zz.TD_Maturity_date,zz.Deposit_amount,yy.cr_gl,tt.ude_value INTEREST, zz.projected_interest
...
---- Batch exécutés et leur status
SELECT distinct eh.branch_date,eh.eoc_batch, decode(eh.eoc_batch_status,'C','COMPLETED','A','ABORTED','NOT RUN') STATUS 
FROM aetb_eoc_programs_history eh 
WHERE eh.branch_date between '01-jul-2018' and '31-dec-2018'
order by eh.branch_date,eh.eoc_batch
