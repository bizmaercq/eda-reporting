...
select ac_branch,user_id,trn_dt,external_ref_no,Narrative,trn_code,max(account_debited) account_debited,max(account_credited) account_credited,lcy_amount 
from
(select h.ac_branch,h.trn_dt,h.external_ref_no,acpks_stmt.fn_get_rtl_narrative_desc(h.trn_ref_no,h.MODULE,'TXN',h.TRN_CODE,'ENG',h.AC_NO,h.AC_BRANCH,h.ac_ccy,h.trn_dt,h.LCY_AMOUNT) ||' / ' || 'Teller Ref No:' ||(SELECT XREF FROM DETB_RTL_TELLER where TRN_REF_NO = h.TRN_REF_NO)
...
and to_char(h.trn_dt,'DD/MM/YYYY') = '&TransactionDate' 
and h.user_id ='&UserId')
group by ac_branch,trn_dt,external_ref_no,Narrative,trn_code, lcy_amount,user_id
