...
from sttm_customer c join sttm_cust_personal p on c.customer_no = p.customer_no
                     ...
                     join sttm_country_isocodes i on i.iso2_code = c.country
where c.customer_type ='I'
and c.checker_dt_stamp >= '01/04/2014' ;


select sc.* from sttm_customer sc
...
rpad(nvl(pr.business_description,' '),100,' ')||'";"'||rpad(nvl(pr.r_address1,' '),30,' ')||'";"'||rpad(nvl(c.address_line2,' '),30,' ')||'";"'||rpad(nvl(c.address_line3,' '),30,' ')||'";'||null||';"'||rpad(substr(c.loc_code,1,2),5,' ')||'";"S.'||decode(substr(m.cust_mis_1,1,3),'SEC999','1442',substr(m.cust_mis_1,1,3))||'";"'||rpad(c.fax_number,20,' ')||'";"'||rpad(p.telephone,20,' ')||'";"'||null||'";"'||null||'";"'||
null||'";"'||decode(m.cust_mis_3,'GRP999','90.3',m.cust_mis_3)||' "'
from sttm_customer c join sttm_cust_personal p on c.customer_no = p.customer_no
                     join mitm_customer_default m on c.customer_no = m.customer
...
(
 ...
 case when central_bank_code in ('C71','C72','C75','C76','C86','C42','C43') then trunc(abs (nvl(sum(Amount),0))/1000000,2) end as Decouverts,
 ...
 case when central_bank_code in ('C01','C02','C03','C04','C05','C06','C07','C08','C4G','C4H','C4J') then trunc(abs (nvl(sum(Amount),0))/1000000,2) end as Credit_Long_Terme ,
...
FROM LCTB_CONTRACT_MASTER A,CSTM_PRODUCT C, lctb_ffts D, cstb_contract cs,mitm_customer_default m 
...
AND   A.EVENT_SEQ_NO = (SELECT MAX(D.EVENT_SEQ_NO) FROM LCTB_CONTRACT_MASTER D WHERE D.CONTRACT_REF_NO=A.CONTRACT_REF_NO) and
