  ...
  from (select eod_date,
          ...
          from (select x.eod_date,
                ...
                select i.eod_date,
                  ...
                  left join aetm_eoc_group_branches j on (i.branch_code = j.branch_code and j.group_code in (select a.group_code from aetm_eoc_group_master a where a.record_stat='O'))
                 group by i.eod_date)
...
PROMPT full batch timings with group code for 11x sites
  ...
  select *
   from (select x.eod_date,
 			...
 			left join aetm_eoc_group_branches y on (x.branch_code = y.branch_code and y.group_code in (select a.group_code from aetm_eoc_group_master a where a.record_stat='O'))
 		union all	
 			select i.eod_date,
 				j.group_code,
 				i.branch_code,
...
PROMPT  eitm modules installed
 
 select * from eitm_modules_installed where record_stat='O';
