	 ...
	 BOOK_DATE
      ...
      from (select ac_no,
              ...
              'R','1'))
                                             ...
                                             '' MODULE,
                                             '' IB
                               from xafnfc.sttb_account B
                              WHERE
								ac_or_gl = 'A'
								and exists ( select 1 from xafnfc.acvw_all_ac_entries A 
										where A.ac_no = b.AC_GL_NO 
											and A.ac_branch = b.branch_code
											and A.balance_upd = 'U' 
											and NVL(A.IB, 'N') = 'N' 
											and A.FINANCIAL_CYCLE = '&FINANCIAL_CYCLE'
											and A.period_code <= '&REPORT_PERIOD_CODE'))));
begin
