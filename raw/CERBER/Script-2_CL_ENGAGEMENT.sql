    ...
    select ac_no,
      ...
      from (select ac_no,
              ...
              from (select distinct RELATED_ACCOUNT ac_no,
                                    ...
                                    s.AC_CCY,
                                    xafnfc.bipks_report_loan.fn_get_loan_intrate(RELATED_ACCOUNT) rate,
                                    xafnfc.BIPKS_REPORT_LOAN.fn_get_Principal_arrears_ASdt(RELATED_ACCOUNT,
                      ...
                      from xafnfc.acvw_all_ac_entries s,
                           (SELECT A.ACCOUNT_NUMBER, A.GL_CODE, D.CUSTOMER_ID
                              from xafnfc.CLTB_ACCOUNT_COMP_BAL_BREAKUP A,
                                   xafnfc.CLTB_ACCOUNT_MASTER           D
                             where A.ACCOUNT_NUMBER = D.ACCOUNT_NUMBER
                               ...
                               AND CREATION_DATE = (SELECT MAX(CREATION_DATE) FROM xafnfc.CLTB_ACCOUNT_COMP_BAL_BREAKUP WHERE ACCOUNT_NUMBER = A.ACCOUNT_NUMBER AND CREATION_DATE <= '&REPORT_MONTH_END_DT' AND
