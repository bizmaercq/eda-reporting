  ...
  cursor c1 is
    ...
    select ac_no,
      ...
      from (select ac_no,
                   ...
                   ib,
                   trn_dt,BOOK_DATE
           --(SELECT BOOK_DATE FROM CLTB_ACCOUNT_MASTER WHERE ACCOUNT_NUMBER = a.account_number ) BOOK_DATE 
              from (select distinct RELATED_ACCOUNT ac_no,
                                    ...
                                    decode((select distinct resident_status
                                           ...
                                           'N',
                                           '0','R','1') RESIDENCE,
                                     (select NN.CUST_MIS_1 from mitm_customer_default NN WHERE NN.CUSTOMER = EE.CUSTOMER_ID
) INSTITUTIONAL_UNIT,
((SELECT decode(SUBSTR(LOC_CODE,1,2),'10','101',
   '11','102',
   '12','103',
   '13','104',
   '14','102',
   '15','105',
   '18','106',
   '19','109') FROM STTM_CUSTOMER SS WHERE SS.CUSTOMER_NO =EE.CUSTOMER_ID )) GEOGRAPHICAL_LOCATION,
                                             (SELECT DECODE ( SUBSTR (CUST_MIS_3,1,1),'G','9', SUBSTR
                                    ...
                                    '' ib,
                                    '' trn_dt,
                                    (SELECT BOOK_DATE FROM CLTB_ACCOUNT_MASTER WHERE ACCOUNT_NUMBER = S.RELATED_ACCOUNT ) BOOK_DATE 
