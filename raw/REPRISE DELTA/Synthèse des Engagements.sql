...
from mouvement2011 join compte2011  on (MVTW_CHA=comw_cha and mvtw_cli=comw_cli and mvtw_suf = comw_suf)
...
/*having (sum(Courant) is not null) 
...
--- Analyse des engagements par type de crédit

select mvtw_age,MVTW_CLI,cliw_int,
 case when sum(COURANT)<0 then sum(COURANT) else null end as Courant
, case when sum(CREDIT_COURT)<0 then sum(CREDIT_COURT) else null end as Credit_Court
, case when sum(CREDIT_MOYEN)<0 then sum(CREDIT_MOYEN) else null end as Credit_Moyen
, case when sum(CREDIT_LONG)<0 then sum(CREDIT_LONG) else null end as "CREDIT_LONG"
, sum (Douteux) as Douteux
from 
( 
select mvtw_age,mvtw_cli,cliw_int,mvtw_cha,
case when mvtw_cha between '37100' and '37299' then nvl(sum(decode(mvtw_sen,'C', mvtw_mon,0)),0) - nvl(sum(decode(mvtw_sen,'D', mvtw_mon,0)),0) end as Courant,
case when mvtw_cha between '32000' and '32999' then nvl(sum(decode(mvtw_sen,'C', mvtw_mon,0)),0) - nvl(sum(decode(mvtw_sen,'D', mvtw_mon,0)),0) end as Credit_Court,
case when mvtw_cha between '31000' and '31999' then nvl(sum(decode(mvtw_sen,'C', mvtw_mon,0)),0) - nvl(sum(decode(mvtw_sen,'D', mvtw_mon,0)),0) end as Credit_Moyen,
case when mvtw_cha between '30000' and '30999' then nvl(sum(decode(mvtw_sen,'C', mvtw_mon,0)),0) - nvl(sum(decode(mvtw_sen,'D', mvtw_mon,0)),0) end as Credit_Long,
case when mvtw_cha between '34400' and '34599' then nvl(sum(decode(mvtw_sen,'C', mvtw_mon,0)),0) - nvl(sum(decode(mvtw_sen,'D', mvtw_mon,0)),0) end  as Douteux   
from mouvement2011 join compte2011  on (MVTW_CHA=comw_cha and mvtw_cli=comw_cli and mvtw_suf = comw_suf)
     join  clients  on comw_cli = cliw_cli
where mvtw_cli >='000999'
...
--having ( (sum(courant) <0) or (sum(credit) <0) )
having (sum(Courant) is not null) 
or (sum(Credit_Court) is not null) 
or(sum(Credit_Moyen) is not null)
or(sum(Douteux) is not null) 
or(sum(Credit_Long) is not null) ;
