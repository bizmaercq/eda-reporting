...
select * from sttms_aeod_dates order by branch_code; 
...
select * from eitbs_pending_programs  order by branch_code;
...
select * from aetms_process_defn  order by branch_code;
...
select * from AETMS_PROCESS_SCHEDULE;
...
SELECT * FROM aetbs_func_detail ;

SELECT * FROM aetbs_func_master;
...
select * from fxtbs_batch_reversal_chk;

--FCJ Version - eoc_runchart, eoc_programs should be blank before start of eod
...
select * from FBTM_BRANCH_INFO;

select * from FBTB_DATES;

select * from FBTM_LCL_HOLIDAY;


select * from user_objects where object_name in ('FN_BUILD_INT_GLS')
/
