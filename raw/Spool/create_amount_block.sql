...
select acy_blocked_amount, a.*
 ...
 where cust_ac_no in (select distinct cm.dr_account                                      
                       ...
                       where cu.customer_no = substr(cm.dr_account, 9, 6)
                         ...
                         and cm.si_expiry_date >= '15/12/2012'
                         and ci.next_exec_date >= '13/12/2012'
                         and cm.max_retry_count = 10);
						 
 select CASQ_AMOUNT_BLOCK.nextval from dual;					 
...
values
  (each_row.dr_acc_br,
   each_row.dr_account,
   'AB'||CASQ_AMOUNT_BLOCK.nextval,
   each_row.si_amt,
   to_date('18-12-2012', 'dd-mm-yyyy'),
...
select *
 ...
 where account in (select cm.dr_account
                    ...
                    where cu.customer_no = substr(cm.dr_account, 9, 6)
                      ...
                      and ci.next_exec_date >= '13/12/2012'
                      and cm.max_retry_count = 10)
   and effective_date>=to_date('18-12-2012', 'dd-mm-yyyy');
