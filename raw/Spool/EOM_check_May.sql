...
SELECT * FROM xafnfc.sttb_account WHERE gl_category IS NULL AND ac_gl_Rec_status <> 'C' and ac_or_gl<>'L'
/
...
SELECT * FROM xafnfc.STTM_CUST_ACCOUNT WHERE (dr_gl IS NULL OR CR_GL IS NULL) AND RECORD_STAT='O' AND AUTH_STAT='A'
/
...
SELECT *
  FROM xafnfc.gltm_glmaster g
 WHERE g.leaf = 'Y'
   AND g.customer <> 'C'
   AND g.record_stat <> 'C'
   AND G.AUTH_STAT='A'
   AND NOT EXISTS (SELECT 1
          FROM xafnfc.sttb_account s
         WHERE s.ac_or_gl = 'G'
...
SELECT BRN "BRANCH CODE", CCY1, CCY2
  FROM xafnfc.ICTB_CCY_PAIR A
 WHERE NOT EXISTS (SELECT '1'
          FROM xafnfc.CYTM_RATES
...
FROM xafnfc.ICTM_ACC I, xafnfc.STTM_CUST_ACCOUNT T
...
FROM xafnfc.CSTBS_PARAM  WHERE PARAM_NAME = 'IC50'
/
...
FROM xafnfc.ICTM_PR_INT 
WHERE PRODUCT_CODE IN(SELECT PRODUCT_CODE FROM xafnfc.CSTM_PRODUCT WHERE RECORD_STAT='O' AND MODULE='IC') 
...
SELECT * FROM xafnfc.CSTB_COMMITFREQ WHERE MODULE_ID='IC' AND FUNCTION_ID='ICEOD'
/
...
FROM xafnfc.ICTM_BRANCH_PARAMETERS 
WHERE BRANCH_CODE IN (SELECT BRANCH_CODE FROM xafnfc.STTM_BRANCH WHERE RECORD_STAT='O')
	and LIQ_NETTING ='Y'
/
...
SELECT * FROM xafnfc.ICTB_ACC_ACTION 
...
FROM xafnfc.ICTB_ACC_PR , xafnfc.STTM_DATES D 
WHERE PROD_TYPE = 'C'  
...
SELECT s.*, l.*
FROM xafnfc.sttm_cust_account s, xafnfc.sttm_customer s1, xafnfc.lmtm_limits l,xafnfc.sttm_dates dt
...
AND l.line_expiry_date < ( select min(pr.pc_start_date) from xafnfc.sttm_period_codes pr where pr.pc_start_date > dt.today)
 ...
 WHERE s.line_id is not null
   ...
   AND l.line_expiry_date <
       (select min(pr.pc_start_date)
          from xafnfc.sttm_period_codes pr
         where pr.pc_start_date > dt.today)
   AND l.availability_flag = 'Y'
   AND l.record_stat = 'O'
   AND NOT EXISTS (SELECT *
          from xafnfc.lmtb_line_utils
         WHERE is_account = 'Y'
...
SELECT L.LIAB_BR,
       ...
       L.UTILISATION
  FROM xafnfc.STTM_CUST_ACCOUNT A,
       xafnfc.STTM_CUSTOMER     C,
       xafnfc.LMTM_LIMITS       L,
       xafnfc.sttm_branch       S,
       xafnfc.sttm_period_codes P
