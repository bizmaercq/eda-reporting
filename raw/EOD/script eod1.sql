...
and f.functionid not in ('TVCL', 'TVQR', 'EODM')
and b.function_id = F.functionid
and F.POSTINGDATE IN ('31/05/2012')
...
(
  ...
  where branch_code not in (SELECT D.BRANCH_CODE
   ...
   MINUS 
   select branch_code from xafnfc.sttb_brn_eodm where run_date = '08-MAY-2012'    
)               

-- EOD Completed for below---
select * from xafnfc.sttb_brn_eodm where run_date = '08-MAY-2012'
