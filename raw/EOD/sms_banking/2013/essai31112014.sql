...
and financial_cycle ='&FYEAR'
and ca.account_class in ('282','281')
--and drcr_ind = 'D'
...
select distinct cm.contract_ref_no,cu.customer_name1, cm.dr_account, cm.si_amt
from sitb_cycle_detail cd, sitb_contract_master cm, sttm_customer cu
...
and cm.contract_ref_no not in (select dd.contract_ref_no from sitb_cycle_detail dd where dd.event_code = 'SUXS' and dd.retry_date between '01/03/2014' and '31/03/2014')

--- Standing instruction programmés pour une période

select m.dr_acc_br,m.contract_ref_no,c.customer_name1,m.dr_account,m.si_amt,i.book_date  
...
and i.book_date between '01/03/2014' and '31/03/2014'
